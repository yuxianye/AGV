...
namespace Solution.Core.Data
{
    public class CreateDatabaseSeedAction : ISeedAction
    {
        ...
        public void Action(DbContext context)
        {
            ...
            //增加初始化数据 角色：管理员、教师、学生，用户：admin、teacher1、student1,默认密码yuxianye1
            //管理员角色和用户
            ...
            context.Set<UserRoleMap>().Add(new UserRoleMap()
            {
                ...
                BeginTime = DateTime.Now,
                EndTime = DateTime.MaxValue,
                Role = new Role() { Name = "教师", Remark = "教师角色", IsAdmin = false, IsSystem = true, CreatedTime = DateTime.Now },
                User = context.Set<User>().Add(new User()
                {
                    Id = 2,
                    AccessFailedCount = 0,
            ...
            context.Set<UserRoleMap>().Add(new UserRoleMap()
            {
                ...
                BeginTime = DateTime.Now,
                EndTime = DateTime.MaxValue,
                Role = new Role() { Name = "学生", Remark = "学生角色", IsAdmin = false, IsSystem = true, CreatedTime = DateTime.Now },
                User = context.Set<User>().Add(new User()
                {
                    Id = 3,
                    AccessFailedCount = 0,
