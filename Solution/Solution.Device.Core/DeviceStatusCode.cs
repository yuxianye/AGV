...
namespace Solution.Device.Core
{
    ...
    /// OPC UA 状态吗
    /// </summary>
    public static class UaStatusCodes
    {
        /// <summary>
        ...
        {
            FieldInfo[] fields = typeof(UaStatusCodes).GetFields(BindingFlags.Public | BindingFlags.Static);

            foreach (FieldInfo field in fields)
            {
                if (identifier == (uint)field.GetValue(typeof(UaStatusCodes)))
                {
                    return field.Name;
                }
            }
        ...
        public static uint GetIdentifier(string browseName)
        {
            ...
            foreach (FieldInfo field in fields)
            {
                ...
                {
                    return (uint)field.GetValue(typeof(UaStatusCodes));
                }
            }
            return 0;
        }
